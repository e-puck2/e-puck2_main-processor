...
#define RAW16BITS_TO_TESLA  (4912.0/32760.0)  // Measurement range of each axis is [-32760..32760] in 16-bit output, with magnetic flux ranging from 4912 to -4912 (see "icm20948 register map").
...
ICM_20948_Device_t icmDevice = { ._last_bank = 4 };
ICM_20948_Status_e icm_write_i2c(uint8_t reg, uint8_t *data, uint32_t len, void *user);
ICM_20948_Status_e icm_read_i2c(uint8_t reg, uint8_t *buff, uint32_t len, void *user);
ICM_20948_Serif_t icmSerif = {
    icm_write_i2c, // write
    icm_read_i2c,  // read
    NULL,
};
...
{
	int8_t err = 0;
	err = write_reg_multi(ICM_20948_I2C_ADDR_AD0, reg, data, len);
	if (err == MSG_OK) {
...
{
	int8_t err = 0;
	err = read_reg_multi(ICM_20948_I2C_ADDR_AD0, reg, buff, len);
	if (err == MSG_OK) {
		return ICM_20948_Stat_Ok;
	} else {
		return ICM_20948_Stat_Err;
...
ICM_20948_Status_e ICM_20948_get_FIFO_count(ICM_20948_Device_t *pdev, uint16_t *count)
{
  ...
  ctrlh.ICM_FIFO_COUNTH &= 0x1F; // Datasheet says "FIFO_CNT[12:8]"
  ...
  *count = (((uint16_t)ctrlh.ICM_FIFO_COUNTH) << 8) | (uint16_t)ctrll.ICM_FIFO_COUNTL;
...
int8_t icm20948_read(ICM_20948_Device_t *pdev, float *gyro, float *acc, float *temp, float *magnet, int16_t *gyro_raw,
					int16_t *acc_raw, int16_t *gyro_offset, int16_t *acc_offset, uint8_t *status)
{
	int8_t err = 0;
	static uint8_t reset_fifo = 0;

	//set_led(LED5, 2);

	// Get data from fifo: accel (6), gyro (6), magnetometer (9 => from ST1 to ST2)
	static uint8_t buf[21] = {0};
	...
	if(reset_fifo == 1)
	{
		reset_fifo = 0;
		i2c_temp[0] = 0x00;
		ICM_20948_execute_w(pdev, AGB0_REG_FIFO_RST, i2c_temp, 1);
	}

	err = ICM_20948_execute_r(pdev, AGB0_REG_FIFO_COUNT_H, i2c_temp, 2);
	fifo_count = (i2c_temp[0]<<8) + i2c_temp[1];
	//chprintf((BaseSequentialStream *)&SDU1, "fifo cnt=%d\r\n", fifo_count);
	if(fifo_count < 21)
	{
		return 0;
	}

	err = ICM_20948_execute_r(pdev, AGB0_REG_INT_STATUS_2, i2c_temp, 1);
	if(i2c_temp[0] == 1) // FIFO overflow
	{
		i2c_temp[0] = 0x01; // Reset fifo
		ICM_20948_execute_w(pdev, AGB0_REG_FIFO_RST, i2c_temp, 1);
		reset_fifo = 1;
		return 0;
	}

	err = ICM_20948_execute_r(pdev, AGB0_REG_FIFO_R_W, buf, sizeof(buf));
    ...
    if (acc) {
    	...
    	acc_raw[X_AXIS] = -read_word(&buf[0]);
    	acc_raw[Y_AXIS] = -read_word(&buf[2]);
    	acc_raw[Z_AXIS] = -read_word(&buf[4]);
    ...
    if (gyro) {
    	gyro_raw[X_AXIS] = read_word(&buf[6]);
    	gyro_raw[Y_AXIS] = read_word(&buf[8]);
    	gyro_raw[Z_AXIS] = read_word(&buf[10]);
    ...
    if (temp) {
        *temp = (float)((read_word(&buf[12]) - 21.0f) / 333.87f) + 21.0f; // Degrees.
    }
    if(magnet){
        magnet[X_AXIS] =  ((int16_t)((int8_t)buf[14]) << 8 | buf[13]) * RAW16BITS_TO_TESLA;
        magnet[Y_AXIS] = ((int16_t)((int8_t)buf[16]) << 8 | buf[15]) * RAW16BITS_TO_TESLA;
        magnet[Z_AXIS] = ((int16_t)((int8_t)buf[18]) << 8 | buf[17]) * RAW16BITS_TO_TESLA;
    }
