...
#if HAL_USE_I2C1_MS_SLAVE || HAL_USE_I2C_LOCK
/**
...
static void i2c_lld_safety_timeout(void *p) {
  ...
  stopTimer(i2cp);
#if HAL_USE_I2C1_MS_SLAVE             /* abort any slave operation in progress */
...
qEvt(0xee,errCode);
  ...
  i2cp->errors = errCode;
  if(errCode == I2C_STOPPED) {
	  do {
	    osalThreadResumeI(&(i2cp)->thread, I2C_ERROR);
	  } while(0);
  } else {
